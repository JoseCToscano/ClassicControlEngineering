...
for x in range(0,len(testing_points)):
    ...
    pdf.cell(200, 10,"Angle for testing point " + str(testing_point) + " is: " + str(angle_of_testing_point), 0, 2, 'L')
    if (angle_of_testing_point/180)%2 == 1:
        print("the point" + str(testing_point) + " does belong to the root loccus")
        pdf.cell(200, 10,"the point" + str(testing_point) + " does belong to the root loccus", 0, 2, 'L')
        num_evaluated=1
        den_evaluated=1
        for i in range(0,len(num)):
            real_part = -np.real(num[i])+np.real(testing_point)
            imag_part = -np.imag(num[i])+np.imag(testing_point)
            sum_value = np.complex(real_part,imag_part)
            num_evaluated *= sum_value
        for i in range(0,len(den)):
            real_part = -np.real(den[i])+np.real(testing_point)
            imag_part = -np.imag(den[i])+np.imag(testing_point)
            sum_value = np.complex(real_part,imag_part)
            den_evaluated *= sum_value
        response = control.evalfr(G, testing_point)
        value_of_K_testing_point = round(1/abs(response),4)
        magnitud_K = (1/abs(response))
        print("The magnitud of K corresponding to the test point " + str(testing_point) + " is " + str(magnitud_K))
        pdf.cell(200, 10,"The magnitud of K corresponding to the test point " + str(testing_point) + " is " + str(magnitud_K), 0, 2, 'L')
    else:
        ...
        pdf.set_font('arial', 'B', 12.0)
        pdf.cell(200, 10, "Lead Controler Design:", 0, 2, 'L')
        pdf.set_font('arial', '', 11.0)
        pdf.cell(200, 10,"the point" + str(testing_point) + " does not belong to the root loccus. The angle deficit is: " + str(angle_defficit) + " degrees, or " + str(round(math.radians(angle_defficit),4)) + " radians.", 0, 2, 'L')
        ...
        pdf.cell(200, 10,"The zero of the Lead Controler is in S=" +str(new_zero), 0, 2, 'L')
        print("The pole of the Lead Controler is in S=" +str(new_pole))
        pdf.cell(200, 10,"The pole of the Lead Controler is in S=" +str(new_pole), 0, 2, 'L')
...
pdf.output('HomeWork_LeadControler_1.pdf', 'F')
